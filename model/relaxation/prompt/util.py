...
def llama_compute_query_states(model: nn.Module, rotary_emb: nn.Module, **kwargs) -> torch.Tensor:
    """
    ...
    if "position_ids" not in inspect.signature(rotary_emb.forward).parameters:
    ...
    if "seq_len" in inspect.signature(rotary_emb.forward).parameters:
        rotary_emb_kwargs["seq_len"] = q_len + past_seen_tokens

    cos, sin = rotary_emb(value_states, **rotary_emb_kwargs)
