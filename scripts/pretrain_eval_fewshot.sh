...
for config_name in "$@"; do
  ...
  if [ "$launcher_type" == "deepspeed" ]; then
    ...
    HYDRA_FULL_ERROR=1 accelerate launch --config_file acc_configs/single_gpu_config.yaml --gpu_ids $first_gpu --num_processes 1 --main_process_port "$random_port" eval_fewshot.py --config-name "$config_name"
  fi
done
